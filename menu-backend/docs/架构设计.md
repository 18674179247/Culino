...
2. 实现方（如 `menu-ai`）提供 trait 实现
3. `main.rs` 在构建 `AppState` 时注入具体实现
4. 调用方（如 `menu-social`）从 `state.behavior_logger` 获取 trait object
...
|------|-------|-----------|------------|------|
| user | menu-user | ❌ | ✅ | 用户注册/登录/资料 |
| recipe | menu-recipe | ✅ | ✅ | 菜谱 CRUD + 搜索 + 随机 |
| ingredient | menu-ingredient | ❌ | ✅ | 食材/调料/标签管理 |
| social | menu-social | ❌ | ✅ | 收藏 + 烹饪记录 |
| tool | menu-tool | ❌ | ✅ | 购物清单 + 膳食计划 |
| upload | menu-upload | ❌ | ❌ | S3 图片上传/删除 |
| ai | menu-ai | ✅ | ❌ | 营养分析/推荐/偏好 |
