# Menu Mobile — KMP 多平台移动端技术设计

## 概述

基于 menu-backend REST API 构建对应的移动端应用，使用 Kotlin Multiplatform (KMP) + Compose Multiplatform 实现 Android、iOS、HarmonyOS NEXT 三端统一开发。覆盖后端全部 API 功能，包含消费者端和管理端。
