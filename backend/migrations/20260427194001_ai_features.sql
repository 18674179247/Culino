...
-- 营养分析更新时间触发器
CREATE
OR REPLACE FUNCTION update_nutrition_timestamp()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at
= now();
...
-- 用户偏好更新时间触发器
CREATE
OR REPLACE FUNCTION update_preference_timestamp()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at
= now();
...
COMMENT
ON TABLE recipe_nutrition IS 'AI 自动生成的菜谱营养分析数据';
COMMENT
ON TABLE user_preferences IS '用户偏好画像，基于行为数据 AI 分析生成';
COMMENT
ON TABLE ai_recommendations IS 'AI 推荐记录，用于追踪推荐效果';
COMMENT
ON TABLE user_behavior_logs IS '用户行为日志，用于推荐系统训练';

COMMENT
ON COLUMN recipe_nutrition.calories IS '每份热量（千卡）';
COMMENT
ON COLUMN recipe_nutrition.health_score IS '健康评分 1-100，越高越健康';
COMMENT
ON COLUMN user_preferences.favorite_cuisines IS 'JSONB 格式：{"川菜": 0.8, "粤菜": 0.6}';
COMMENT
ON COLUMN ai_recommendations.recommendation_type IS '推荐类型：personalized(个性化)/similar(相似)/trending(热门)/health_goal(健康目标)';
COMMENT
ON COLUMN user_behavior_logs.action_type IS '行为类型：view/favorite/unfavorite/cook/rate/search/share';
